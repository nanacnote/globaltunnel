...
const argv = optimist
  ...
  .options("domain", {
    default: "localhost",
    describe:
      "Specify the base domain name. This is optional if hosting globaltunnel from a regular example.com domain. This is required if hosting a globaltunnel server from a subdomain (i.e. gt.example.com where clients will be client-app.gt.example.come)",
  })
...
InitialiseHTTPServer(argv).then((server) => {
  process.on("SIGINT", () => {
    server.close(() => {
      process.exit();
    });
